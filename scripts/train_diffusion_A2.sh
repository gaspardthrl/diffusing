#!/bin/bash
# A2 — Gripper-only proprio (relative actions, model sees only gripper state)
# Purpose: remove joint position shortcut, force vision-driven generalization.
# The model is built with state_dim=1 (gripper only); arm joint deltas still
# computed relative to full state in preprocessing (not visible to model).
#
# Usage:
#   HF_REPO_ID=user/repo ./scripts/train_diffusion_A2.sh [STEPS]
set -e

STEPS="${1:-200000}"
DATASET_REPO_ID="${DATASET_REPO_ID:-gaspardthrl/walleed_fold_combined}"
OUTPUT_DIR="outputs/diffusion_A2_$(date +%Y%m%d_%H%M%S)"
...
uv run lerobot-train \
  ...
  --job_name="diffusion-A2-gripper-only" \
  --wandb.notes="A2 gripper-only proprio, relative actions, MEAN_STD action norm, ResNet-18, ${STEPS} steps" \
  \
