#!/bin/bash
# A4 — Vision-only (no proprio, relative actions still applied in preprocessing)
# Purpose: fully state-free baseline; tests whether vision alone is sufficient.
# Model is built with state_dim=0: no state conditioning pathway at all.
#
# Usage:
#   HF_REPO_ID=user/repo ./scripts/train_diffusion_A4.sh [STEPS]
set -e

STEPS="${1:-100000}"
DATASET_REPO_ID="${DATASET_REPO_ID:-gaspardthrl/walleed_fold_combined}"
OUTPUT_DIR="outputs/diffusion_A4_$(date +%Y%m%d_%H%M%S)"
...
uv run lerobot-train \
  ...
  \
  `# ── A4: no state conditioning, relative actions still applied ─────` \
  ...
  --job_name="diffusion-A4-vision-only" \
  --wandb.notes="A4 vision-only (no proprio), relative actions, MEAN_STD action norm, ResNet-18, ${STEPS} steps" \
  \
