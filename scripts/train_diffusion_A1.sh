#!/bin/bash
# A1 — Full proprio baseline (all 6 joints + gripper, relative actions)
# Purpose: true relative-action baseline; gate for A2-A4.
#
# Usage:
#   HF_REPO_ID=user/repo ./scripts/train_diffusion_A1.sh [STEPS]
...
# Download relative_stats.json if not present locally
...
uv run lerobot-train \
  ...
  \
  `# ── A1: full proprio, relative actions ───────────────────────────` \
  ...
  --job_name="diffusion-A1-full-proprio" \
  --wandb.notes="A1 full proprio baseline, relative actions, MEAN_STD action norm, ResNet-18, ${STEPS} steps" \
  \
